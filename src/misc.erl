...
datetime_to_string({{Year,Month,Day},{Hour,Min,Sec}})->
    ...
    _WeekDayStr=httpd_util:day( calendar:day_of_the_week(Year,Month,Day) ),
...
cnt_distrib(Data, Distrib)->
    _NewDistrib=case lists:keyfind(Data, 1, Distrib) of
...
list_to_term(List)->
    {ok, Tokens, _EndLoc}=erl_scan:string(List++"."),
...
%%%------------------------------------------------------------------------------------------------
%%% load a module on a specific node / all connected nodes
%%% TODO: verify that code is actually loaded (check return from rpc:call)
%%%------------------------------------------------------------------------------------------------
network_load(Module)->
